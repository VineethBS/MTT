function [Matching, Cost] = hungarian_algorithm(o, cost_matrix)
...
Matching = zeros(size(cost_matrix));
...
num_y = sum(~isinf(cost_matrix),1);
% Find the number in each row that are connected
num_x = sum(~isinf(cost_matrix),2);
...
P_cond = zeros(P_size);
P_cond(1:length(x_con),1:length(y_con)) = cost_matrix(x_con,y_con);
if isempty(P_cond)
    Cost = 0;
    return
end
...
Cost = sum(sum(cost_matrix(Matching==1)));
